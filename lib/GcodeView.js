class GcodeView {
  ...
  constructor(options)
  {
    ...
    this.MachineExtents = {x: 45.5, y: 45.5};
    this.WorkOffset = {x: 0, y: 0};
		//Hander bits
	...
	init()
  {
    $("#Content").html("<svg style='width: " + ($(window).width() - 30) + "px; height: " + ($(window).height() - 30) + "px;' id='draw'><g id='draw-group'></g></svg><div id='draw-info' style='position: absolute; bottom: 20px; right: 40px; font-weight: bold;'></div><input id='draw-search' size='35' style='position: absolute; bottom: 20px; left: 80px; font-weight: bold;'></input>");
    ...
    this.LatheMode = false;
		...
		if (!this.HandlersAlreadyAttached)
		{
			...
			setInterval(this.RenderLoop.bind(this), 250);
  ...
  render_orig(live)
  {
    ...
    svg += this.svgMachineBorder("red");
    svg += this.svgCrosshair(this.CrossHairPosition, this.View.MouseOverRadius / this.View.zoom,"blue");
		this.View.svg = svg;
    ...
    for (var x = 0; x < this.Stack.length; x++)
		{
			...
			if (this.Stack[x].type == "line")
			{
				this.EntitiesSVG += this.svgLine({ x: this.Stack[x].origin[0], y: this.Stack[x].origin[1] }, { x: this.Stack[x].end[0], y: this.Stack[x].end[1] }, color, "solid");
			}
			if (this.Stack[x].type == "circle")
			{
				this.EntitiesSVG += this.svgCircle({ x: this.Stack[x].origin[0], y: this.Stack[x].origin[1] }, this.Stack[x].radius, color, "solid");
			}
			if (this.Stack[x].type == "arc")
			{
				this.EntitiesSVG += this.svgArc({ x: this.Stack[x].origin[0], y: this.Stack[x].origin[1] }, this.Stack[x].radius, this.Stack[x].startAngle, this.Stack[x].endAngle, color, "solid");
			}
		}
  }
  render(entities)
  {
    ...
    svg += this.EntitiesSVG;
    svg += this.svgMachineBorder("red");
    svg += this.svgCrosshair(this.CrossHairPosition, this.View.MouseOverRadius / this.View.zoom,"blue");
		this.View.svg = svg;
		document.getElementById("draw-group").innerHTML = svg;
  }
  ...
  breakLink(obj)
  {
	...
	{
    origin.x -= this.WorkOffset.x;
    origin.y -= this.WorkOffset.y;
		if (style === "dashed")
		{
  ...
  svgMachineBorder(color)
	{
		var markup = "<line x1='" + 0 + "' y1='" + 0 + "' x2='" + this.MachineExtents.x + "' y2='" + 0 + "' stroke='" + color + "' stroke-width='" + this.View.line_width + "' vector-effect='non-scaling-stroke' fill='none'/>\n";
    markup += "<line x1='" + this.MachineExtents.x + "' y1='" + 0 + "' x2='" + this.MachineExtents.x + "' y2='" + this.MachineExtents.y + "' stroke='" + color + "' stroke-width='" + this.View.line_width + "' vector-effect='non-scaling-stroke' fill='none'/>\n";
    markup += "<line x1='" + this.MachineExtents.x + "' y1='" + this.MachineExtents.y + "' x2='" + 0 + "' y2='" + this.MachineExtents.y + "' stroke='" + color + "' stroke-width='" + this.View.line_width + "' vector-effect='non-scaling-stroke' fill='none'/>\n";
    markup += "<line x1='" + 0 + "' y1='" + this.MachineExtents.y + "' x2='" + 0 + "' y2='" + 0 + "' stroke='" + color + "' stroke-width='" + this.View.line_width + "' vector-effect='non-scaling-stroke' fill='none'/>\n";
		return markup;
	}
    ...
    origin.x -= this.WorkOffset.x;
    origin.y -= this.WorkOffset.y;
    end.x -= this.WorkOffset.x;
    end.y -= this.WorkOffset.y;
		if (style === "dashed")
		{
