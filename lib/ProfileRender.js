...
function initProfileRenderCore (context) {
  ...
  var ProfileRender = context.ProfileRender = function (opt_config) {
    ...
    this.Stack = [
      { part_name: "TestPart", offset: [0, 0, 0], entities: [
        { type: "line", origin: [0, 0, 0], end: [10, 10, 0], color: this._colorWhite },
        { type: "line", origin: [0, 0, 0], end: [-10, 10, 0], color: this._colorWhite },
        { type: "line", origin: [0, 0, 0], end: [0, 0, 10], color: this._colorWhite },
        { type: "arc", origin: [0, 0, 0], radius: 5, startAngle: 0, endAngle: 90, direction: this._dirClockwise, color: this._colorWhite },
  ...
  ProfileRender.prototype.onMouseWheel = function (e){
    var last_zoom = this.camera.position.z;
    if (e.deltaY > 0)
    {
      //Zoom out
      this.camera.position.z += (this.camera.position.z * (1 / 20));
    }
    else
    {
      //Zoom in
    ...
    var scalechange = this.camera.position.z - last_zoom;
    //console.log(scale_change);
    this.lookAt.x += ((this.mousePosition.x * scalechange) * -1);
		this.lookAt.y +=((this.mousePosition.y * scalechange) * -1);
    this.camera.lookAt( this.lookAt.x, this.lookAt.y, this.lookAt.z );
	}
