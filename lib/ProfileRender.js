...
function initProfileRenderCore (context) {
   ...
   */
    ...
    **/
    ...
    ProfileRender.prototype.entity_mouse_interaction = function () {
      ...
      {
        var part = this.Stack[x];
        var offset_mouse = {x: this.mousePosition.x - part.offset[0], y: this.mousePosition.y - part.offset[1]};
        if (part.hidden == false && part.internal == false)
        {
          for (var y = 0; y < part.entities.length; y++)
          {
            ...
            if (entity.type == "line")
            {
              ...
              var line1 = this.geometry.get_line_at_angle({x: offset_mouse.x, y: offset_mouse.y}, angle, (z_factor * this._mouseOverPadding) / 2);
              var line2 = this.geometry.get_line_at_angle({x: offset_mouse.x, y: offset_mouse.y}, angle + 180, (z_factor * this._mouseOverPadding) / 2);
              ...
              if (int)
              {
                if (entity.meta.mouse_over == false) //Only calculate the snap points once!
                {
                  ...
                  snap_entities.offset = [part.offset[0], part.offset[1]];
  ...
  /**
   * Toggle Part Visibility by part name
   * @return {void}
   */
  ProfileRender.prototype.togglePartVisibility = function (part_name) {
    for( var i = 0; i < this.Stack.length; i++)
    {
       if ( this.Stack[i].part_name == part_name)
       {
         this.Stack[i].hidden = !this.Stack[i].hidden;
         this.Stack[i].updateRender = true;
       }
    }
  };
  ...
  ProfileRender.prototype.init = function () {
    ...
    $("canvas").css({"margin-left": this._renderLeftMargin + "px"});
    $("canvas").css({"margin-top": this._renderTopMargin + "px"});
  ...
  ProfileRender.prototype.onMouseMotion = function (e){
    ...
    /*vec.set(
        ...
        0.5 );*/
    vec.set(
        ( ((e.clientX - 6) - this._renderLeftMargin) / this._renderWidth ) * 2 - 1,
        - ( ((e.clientY - 6) - this._renderTopMargin) / this._renderHeight ) * 2 + 1,
        0.5 );
  ...
  ProfileRender.prototype.onMouseClick = function (e){
    ...
    if (e.button == 0) //We are a left button Click
    {
      ...
      this.mouse_click_check(this.mousePosition);
    }
  ...
  ProfileRender.prototype.onMouseWheel = function (e){
    ...
    var mX = ((event.clientX - this._renderLeftMargin) / this._renderWidth) * 2 - 1;
    var mY = -((event.clientY - this._renderTopMargin) / this._renderHeight) * 2 + 1;
